...
#define	error(e) { error = (e); goto bad; }
	...
	/* If no service pathname supplied, provide one */
	char unique_sock_path[1024];//11/15 9pm 
	if (hostname != NULL &&
		(strcmp(hostname, "/local") == 0 || strcmp(hostname, "/unix") == 0))
	{
		if (servname == NULL){
			snprintf(unique_sock_path, sizeof(unique_sock_path), "/tmp/myapp_sock_%ld", (long) getpid());//11/15 9pm
            servname = unique_sock_path;//servname = tmpnam(NULL); 11/15 9pm
		}
		else if (servname[0] == '\0'){
			snprintf(unique_sock_path, sizeof(unique_sock_path), "/tmp/myapp_sock_%ld", (long) getpid());//11/15 9pm
            servname = unique_sock_path;//servname = tmpnam(NULL);
		}
		if (servname[0] == '/')
